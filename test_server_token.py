...
class ServerTokenTests(unittest.TestCase):
    ...
    def test_records_json_bytes_exports_versioned_history(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(payload["version"], "1.17.0")
    ...
    def test_security_headers_are_added_to_every_response(self):
        ...
        self.assertEqual(handler.version_string(), "AssetVIXLocal/1.17.0")
    ...
    def test_web_signal_queue_controls_are_present(self):
        index = (server.WEB_DIR / "index.html").read_text(encoding="utf-8")
        script = (server.WEB_DIR / "app.js").read_text(encoding="utf-8")
        self.assertIn('id="signalQueueSymbols"', index)
        self.assertIn('id="queueSignalsButton"', index)
        self.assertIn("function queueCurrentSignals", script)
        self.assertIn("function loadSignalQueueIntoSymbols", script)

    def test_web_risk_lens_controls_are_present(self):
        index = (server.WEB_DIR / "index.html").read_text(encoding="utf-8")
        script = (server.WEB_DIR / "app.js").read_text(encoding="utf-8")
        self.assertIn('data-pulse-lens="Early watch"', index)
        self.assertIn('data-pulse-lens="Balanced"', index)
        self.assertIn('data-pulse-lens="Stress event"', index)
        self.assertIn("function applyPulseLens", script)
        self.assertIn("function syncPulseLens", script)
